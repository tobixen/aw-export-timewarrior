...
- Fix `diff --apply` and `retag.py` exiting 0 when some of their writes to TimeWarrior failed. `diff --apply` printed a failed fix command and moved on; `retag.py` did the same when the retag rules raised on an interval. Both still work through the rest, but now exit 1, so a script driving them can tell the run was incomplete.
...
- Fix emacs window events being tagged from the wrong file. A sub-event found by the widened lookahead does not overlap the window event, so nothing tied the two together and the longest candidate won — often a same-named file from an unrelated project, which is how a `4OSS` interval ended up tagged as customer work. Such a match is now accepted only if the sub-event's file basename is the buffer named in the window title. Sub-events that genuinely overlap the window event are trusted as before, since a buffer name is not always a file basename.
- Fix emacs window events having no file at all where `activity-watch-mode` had gone quiet — it reports nothing while a buffer is only being read, and gaps of tens of minutes are normal. When the widened lookahead finds nothing, the sub-event immediately before (or after) the window event is now used instead, reaching up to 30 minutes, again only if its file matches the buffer name and only if no sub-event for another file lies in between. Over three weeks of the author's data this filled in 295 previously unmatched events and corrected the file on 314 more.
